...
public class ColorToRgb {
    ...
    private static ColorToRgb colorToRGB;

    private ArrayList<String> wishList = new ArrayList<>();
    private ArrayList<ColorName> hexColors = new ArrayList<>();
    ...
    public static ColorToRgb getInstance() {
        if (colorToRGB == null) {
            colorToRGB = new ColorToRgb();
        }
        return colorToRGB;
    }

    public int nameToColor(String name) throws IllegalArgumentException{
        if(hexColors.isEmpty()){
            initColorList();
        }
        for(int i=0; i<hexColors.size();i++){
            if(hexColors.get(i).name.toLowerCase().replace(" ", "").equals(name.toLowerCase().replace(" ", ""))){
                return Color.rgb(hexColors.get(i).red, hexColors.get(i).green, hexColors.get(i).blue);
            }
        }
        wishList.add(name);
        throw new IllegalArgumentException("I do not know that color");
    }

    public ArrayList<ColorName> getHexColors() {
        return hexColors;
    }
    ...
    private void initColorList() {
        ...
        colorList.add(new ColorName("Daylight", 0xFF, 0xF1, 0xE0));
        ...
        colorList.add(new ColorName("FreshAir", 0xA6, 0xE7, 0xFF));
        ...
        colorList.add(new ColorName("USCGold", 0xFF, 0xCC, 0x00));
        ...
        colorList.addAll(getAllOtherColors());
        hexColors = colorList;
        for(ColorName color: hexColors){
            colorNameList.add(color.name);
        }

    }

    public static ArrayList<ColorName> getAllOtherColors(){
        ArrayList<ColorName> otherColors = new ArrayList<>();
        String otherColorsString="";
        try {
            otherColorsString = IOUtils.toString(ColorToRgb.class.getClassLoader().getResourceAsStream("Colors1"), "UTF-8").replace(System.getProperty("line.separator"), "$").replace("	", "");
        } catch (Exception e) {
            e.printStackTrace();
        }
        //TODO without error handling?
        try {
            while(otherColorsString.length() != 0){
                String colorName = otherColorsString.substring(0, otherColorsString.indexOf("#"));
                String hexColor = otherColorsString.substring(otherColorsString.indexOf("#")+1, otherColorsString.indexOf("#")+7);
                otherColorsString = otherColorsString.substring(otherColorsString.indexOf("$")+1);
                ColorName color = new ColorName(colorName, Integer.parseInt(hexColor.substring(0, 2),16), Integer.parseInt(hexColor.substring(2, 4),16), Integer.parseInt(hexColor.substring(4, 6),16));
    ...
    static class ColorName{
        ...
        public ColorName(String name, int red, int green, int blue){
            ...
            this.green = green;
            this.blue = blue;
            this.hex = Integer.toString(red)+Integer.toString(green)+Integer.toString(blue);
        }
    }
}
